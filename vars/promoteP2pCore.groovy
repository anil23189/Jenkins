...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_p2p_core_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-p2p-core.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy P2pCore ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy P2pCore ${params.branch}"
                            }
                            utils.mvn("clean install -Pall,dev-env-v3,jenkins -DskipTests -Dcom.mastercard.p2p.core.fpe.keystore.file=/fs01/app/key-store/fpe-keystore.jks -Dcom.mastercard.p2p.core.fpe.keystore.password=fpe-keystore-password -Dcom.mastercard.p2p.core.fpe.jks.key.alias=fpejkskey", settings_id)
                        }
                    }
                }
            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy P2pCore ${params.ap_pos_nfc_version}"
                        }
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_p2p_core_version=${params.ap_p2p_core_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e ap_p2p_core_version=\$ap_p2p_core_version"
                        }
                    }
                    ...
                    dir ("ansible-${ansible_branch}") {
                        ...
                        sh """
                    ap_p2p_core_version=\$(cat $WORKSPACE/${component}/pom.xml |grep -m1 '<version>.*</version>' |awk -F'[><]' '{print \$3}')
                    ansible-playbook -i inv/hosts.yml ap-p2p-core.yml -e env=${pipelineParams.env} -c local -e workspace=$WORKSPACE \
