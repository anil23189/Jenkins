...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_security_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-security.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy ApCore ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy ApCore ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_security_version=${params.ap_security_version}"
                            currentBuild.description = "Deploy ApCore ${params.ap_security_version}"
                            
                        }
                        else {
                            ap_security_version = utils.version(component)
                            nexus = "-e nexus_download='false' -e ap_security_version=${ap_security_version}"
                        }
                    }
