...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_mdm_core_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-mdm-core.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy MDMCore ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy MDMCore ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy MDMCore ${params.ap_mdm_core_version}"
                        }
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_mdm_core_version=${params.ap_mdm_core_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e ap_mdm_core_version=\$ap_mdm_core_version"
                        }
                    }
                    ...
                    dir ("ansible-${ansible_branch}") {
                        ...
                        sh """
                    ap_mdm_core_version=\$(cat $WORKSPACE/${component}/pom.xml |grep -m1 '<version>.*</version>' |awk -F'[><]' '{print \$3}')
