...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_pos_nfc_rest_core_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        environment{
        ansible_branch = "ap-pos-nfc-rest-core"
        ...
        component = "ap-pos-nfc-rest-core"
        }
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-pos-nfc-rest-core.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy PosnfcRestCore ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy PosnfcRestCore ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy PosnfcRestCore ${params.ap_pos_nfc_rest_core_version}"
                        }
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_pos_nfc_rest_core_version=${params.ap_pos_nfc_rest_core_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e ap_pos_nfc_rest_core_version=\$ap_pos_nfc_rest_core_version"
                        }
                    }
                    ...
                    dir ("ansible-${ansible_branch}") {
                        ...
                        sh """
                    ap_pos_nfc_rest_core_version=\$(cat $WORKSPACE/${component}/pom.xml |grep -m1 '<version>.*</version>' |awk -F'[><]' '{print \$3}')
                    ansible-playbook -i inv/hosts.yml ap-pos-nfc-rest.yml -e env=${pipelineParams.env} -c local -e workspace=$WORKSPACE \
