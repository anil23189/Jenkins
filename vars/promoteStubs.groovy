        ...
        stages {
            ...
            stage('Build'){
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-stubs.git"
                        script {
                            if (params.commit) {
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e stubs_version=${params.stubs_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e stubs_version=\$stubs_version"
                        }
                    }
