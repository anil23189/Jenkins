...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_p2p_integration_version', defaultValue: '', description: 'ap-p2p-integration-version')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-p2p-integration.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy P2pIntegration ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy P2pIntegration ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy P2pIntegration ${params.ap_p2p_integration_version}"
                        }
                        mw_util = utils.utility_version(params.deploy_util_version, 'deploy_util_version')
                        
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_p2p_integration_version=${params.ap_p2p_integration_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e ap_p2p_integration_version=\$ap_p2p_integration_version"
                        }
                    }
