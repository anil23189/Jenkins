...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'pba_rfp_integration_version', defaultValue: '', description: 'pba_rfp_integration_version')
        ...
        environment{
            ...
            component = "pba-rfp-integration"
            settings_id = "pwba-settings"
            ansible_branch = "pba-rfp-integration"
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/pba-rfp-integration.git"
                        script {
                            if (params.commit) {
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy MIntegration ${params.pba_rfp_integration_version}"
                        }
                        mw_util = utils.utility_version(params.deploy_util_version, 'deploy_util_version')
                        
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e pba_rfp_integration_version=${params.pba_rfp_integration_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false'"
                            pba_rfp_integration_version = utils.version(component)
                        }
                    }
                    ...
                    dir ("ansible-${ansible_branch}") {
                        ...
                        sh """
                            ansible-playbook -i inv/hosts.yml pba-rfp-integration.yml -e env=${pipelineParams.env} -c local -e workspace=$WORKSPACE \
                            -e nexus_user=${git_credentials_usr} -e nexus_pass=${git_credentials_psw} \
                            $mw_util $nexus -e pba_rfp_integration_version=${pba_rfp_integration_version} -e properties_version=${params.properties_version}
                        """
                    }
                }
            }
