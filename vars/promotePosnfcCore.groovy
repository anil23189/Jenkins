...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_pos_nfc_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-pos-nfc-core.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy PosnfcCore ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy PosnfcCore ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        if(!params.branch){
                            currentBuild.description = "Deploy PosnfcCore ${params.ap_pos_nfc_version}"
                        }
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_pos_nfc_version=${params.ap_pos_nfc_version}"
                        }
                        else {
                            nexus = "-e nexus_download='false' -e ap_pos_nfc_version=\$ap_pos_nfc_version"
                        }
                    }
                    ...
                    ansible-playbook -i inv/hosts.yml ap-pos-nfc.yml -e env=${pipelineParams.env} -c local -e workspace=$WORKSPACE \
            ...
            stage('Deploy apps ') {
                steps {
                    dir ('ap-middleware-deployment-utility') {
                        sh """
                        ./runAPUtil.sh -Dconfig_properties_url=$WORKSPACE/properties/config-ap-pos-nfc.properties
                        """
                    }
                }
            }
