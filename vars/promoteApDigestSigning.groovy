...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_digest_signing_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-digest-signing.git"
                        script {
                            if (params.commit) {
                                sh "git checkout ${params.commit}"
                                currentBuild.description = "Deploy Ap Digest Signing ${params.commit}"
                            }
                            else {
                                currentBuild.description = "Deploy Ap Digest Signing ${params.branch}"
                            }
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_digest_signing_version=${params.ap_digest_signing_version}"
                            currentBuild.description = "Deploy Ap Digest Signing Core ${params.ap_digest_signing_version}"

                        }
                        else {
                            ap_digest_signing_version = utils.version(component)
                            nexus = "-e nexus_download='false' -e ap_digest_signing_version=${ap_digest_signing_version}"
                        }
                    }
