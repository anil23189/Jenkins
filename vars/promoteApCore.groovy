...
def call(body) {
    ...
    pipeline {
        ...
        parameters{
            ...
            string(name: 'ap_core_version', defaultValue: '', description: 'set to deploy from nexus')
        ...
        stages {
            ...
            stage('Build') {
                ...
                steps {
                    sh "mkdir ${component}"
                    dir ("${component}"){
                        git branch: "${params.branch}", credentialsId: 'zapp.jenkins.build', url: "http://bitbucket.vocalink.co.uk/scm/zapp/ap-core.git"
                        script {
                            if (params.commit) {
            ...
            stage('Prepare Ansible') {
                steps {
                    script {
                        ...
                        if(!params.branch) {
                            nexus = "-e nexus_download='true' -e ap_core_version=${params.ap_core_version}"
                            currentBuild.description = "Deploy ApCore ${params.ap_core_version}"
                            
                        }
                        else {
                            ap_core_version = utils.version(component)
                            nexus = "-e nexus_download='false' -e ap_core_version=${ap_core_version}"
                        }
                    }
